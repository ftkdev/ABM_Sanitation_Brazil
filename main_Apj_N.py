...
alpha_list = [0.8, 0.9, 1, 1.1, 1.2, 1.3]
...
agent_list = [25, 30, 35, 40, 45, 50]  # [N] Total number of agents bidding
...
for BID_ALPHA in alpha_list:
    for NUMBER_OF_AGENTS in agent_list:
        df_bid_details = pd.DataFrame()
        df_bid_results = pd.DataFrame()
        ...
        # 'Results_temp/Apj_N/Details-{}.csv'.format(
        # FILE_NAME), index=False)
        df_bid_results.to_csv(
            'Results_temp/Apj_N/Results-{}.csv'.format(
                FILE_NAME), index=False)
